...
"use strict";
// this file contains the Gui, which is the wp 'controller'. 
...
var Gui = core.Class.extend({
    ...
    init: function(options){
        ...
        this.chrome.ready.then(function(){
            ...
            self.wp.bind('change:selectedOrder', function(){
                self.show_saved_screen(self.wp.get_order());
            });
        });
    },
    ...
    // This sets up automatic wp exit when open in
    // another tab.
    ...
    select_user: function(options){
        ...
        var list = [];
        for (var i = 0; i < this.wp.users.length; i++) {
            var user = this.wp.users[i];
        ...
        this.show_popup('selection',{
            ...
            is_selected: function(user){ return user === self.wp.get_cashier(); },
        });

        return def.then(function(user){
            if (options.security && user !== options.current_user && user.wp_security_pin) {
                return self.ask_password(user.wp_security_pin).then(function(){
                    return user;
                });
            } else {
                return user;
    ...
    /* ---- Gui: CLOSING THE weighing point ---- */

    close: function() {
        var self = this;
        var pending = this.wp.db.get_orders().length;

        if (!pending) {
            this._close();
        } else {
            this.wp.push_order().always(function() {
                var pending = self.wp.db.get_orders().length;
                if (!pending) {
                    self._close();
                } else {
                    var reason = self.wp.get('failed') ? 
                    ...
                    self.show_popup('confirm', {
                        ...
                        'body':  _t(['Some orders could not be submitted to',
                                     ...
                                     'You can exit the weighing point, but do',
    ...
    _close: function() {
        ...
        this.wp.push_order().then(function(){
            var url = "/web#action=weighing_point.action_client_wp_menu";

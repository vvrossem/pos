odoo.define('weighing_point.popups', function (require) {
"use strict";
...
var PopupWidget = WpBaseWidget.extend({
    ...
    show: function(options){
        ...
        if (this.wp.barcode_reader) {
            this.wp.barcode_reader.save_callbacks();
    ...
    close: function(){
        if (this.wp.barcode_reader) {
            this.wp.barcode_reader.restore_callbacks();
        }
    },

    // hides the popup. keep in mind that this is called in 
    // the initialization pass of the wp instantiation, 
...
var ErrorTracebackPopupWidget = ErrorPopupWidget.extend({
    ...
    show: function(opts) {
        ...
        this.$('.email').off('click').click(function(){
            self.gui.send_email( self.wp.company.email,
                _t('IMPORTANT: Bug Report From Odoo weighing point'),
                self.options.body);
        });
    }
});

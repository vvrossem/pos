...
exports.WpModel = Backbone.Model.extend({
    initialize: function(session, attributes) {
        ...
        this.db = new WpDB();                       // a local database used to search trough products and categories & store pending orders
        ...
        // We fetch the backend data on the server asynchronously. this is done only when the wp user interface is launched,
        // Any change on this data made on the server is thus not reflected on the weighing point until it is relaunched.
        // when all the data has loaded, we compute some stuff, and declare the wp ready to be used. 
    ...
    // releases ressources holds by the model at the end of life of the wpmodel
    destroy: function(){
    ...
    models: [
    {
        ...
        model: 'wp.config',
        fields: [],
        domain: function(self){ return [['id','=', self.wp_session.config_id[0]]]; },
        ...
        domain: function(self){ return [['company_id','=',self.user.company_id[0]],'|', ['groups_id','=', self.config.group_wp_manager_id[0]],['groups_id','=', self.config.group_wp_user_id[0]]]; },
        ...
        domain: function(self){ return [['state', '=', 'open'],['wp_session_id', '=', self.wp_session.id]]; },
    ...
    // returns the user who is currently the cashier for this weighing point
        ...
        this.flush_mutex.exec(function(){
            ...
            transfer.pipe(function(order_server_id){
                ...
                if (order_server_id.length) {
                    self.chrome.do_action('weighing_point.wp_invoice_report',{additional_context:{
...
// Loads openerp models at the weighing point startup.
...
exports.Product = Backbone.Model.extend({
    ...
    //
    // Anything related to UOM can be ignored, the wp will always use
...
exports.Orderline = Backbone.Model.extend({
    ...
    init_from_JSON: function(json) {
        this.product = this.wp.db.get_product_by_id(json.product_id);
        if (!this.product) {
            console.error('ERROR: attempting to recover product ID', json.product_id,
                'not available in the weighing point. Correct the product or clean the browser cache.');
        }
        this.set_product_lot(this.product);
    ...
    set_product_lot: function(product){
        this.has_product_lot = product.tracking !== 'none' && this.wp.config.use_existing_lots;
        ...
        if(quantity === 'remove'){
            ...
            if(unit){
                if (unit.rounding) {
                    ...
                    var decimals = this.wp.dp['Product Unit of Measure'];
    ...
    get_unit: function(){
        ...
        return this.wp.units_by_id[unit_id];
    },
    ...
    can_be_merged_with: function(orderline){
        var price = parseFloat(round_di(this.price || 0, this.wp.dp['Product Price']).toFixed(this.wp.dp['Product Price']));
        if( this.get_product().id !== orderline.get_product().id){    //only orderline of the same product can be merged
            return false;
        }else if(!this.get_unit() || !this.get_unit().is_wp_groupable){
            return false;
    ...
    set_unit_price: function(price){
        this.order.assert_editable();
        this.price = round_di(parseFloat(price) || 0, this.wp.dp['Product Price']);
        this.trigger('change',this);
    },
    get_unit_price: function(){
        var digits = this.wp.dp['Product Price'];
        // round and truncate to mimic _symbol_set behavior
        return parseFloat(round_di(this.price || 0, digits).toFixed(digits));
    },
    get_unit_display_price: function(){
        if (this.wp.config.iface_tax_included === 'total') {
    ...
    get_base_price:    function(){
        var rounding = this.wp.currency.rounding;
    ...
    get_applicable_taxes: function(){
        ...
        var taxes = [];
        for (i = 0; i < this.wp.taxes.length; i++) {
            if (ptaxes_set[this.wp.taxes[i].id]) {
                taxes.push(this.wp.taxes[i]);
            }
        }
        return taxes;
    },
    ...
    get_taxes: function(){
        ...
        for (var i = 0; i < taxes_ids.length; i++) {
            taxes.push(this.wp.taxes_by_id[taxes_ids[i]]);
        }
        return taxes;
    },
    _map_tax_fiscal_position: function(tax) {
        ...
        if (order_fiscal_position) {
            ...
            if (mapped_tax) {
                tax = this.wp.taxes_by_id[mapped_tax.tax_dest_id[0]];
            }
        }
    ...
    compute_all: function(taxes, price_unit, quantity, currency_rounding, no_map_tax) {
        ...
        if (this.wp.company.tax_calculation_rounding_method == "round_globally"){
    ...
    get_all_prices: function(){
        ...
        var all_taxes = this.compute_all(product_taxes, price_unit, this.get_quantity(), this.wp.currency.rounding);
...
exports.Paymentline = Backbone.Model.extend({
    initialize: function(attributes, options) {
        ...
        if (this.cashregister === undefined) {
            throw new Error(_t('Please configure a payment method in your wp.'));
        }
        this.name = this.cashregister.journal_id[1];
    },
    init_from_JSON: function(json){
        this.amount = json.amount;
        this.cashregister = this.wp.cashregisters_by_id[json.statement_id];
        this.name = this.cashregister.journal_id[1];
    },
    //sets the amount of money on this payment line
    set_amount: function(value){
        this.order.assert_editable();
        this.amount = round_di(parseFloat(value) || 0, this.wp.currency.decimals);
        this.trigger('change',this);
    },
    ...
    get_amount_str: function(){
        return field_utils.format.float(this.amount, {digits: [69, this.wp.currency.decimals]});
    },
    set_selected: function(selected){
...
// there is always an active ('selected') order in the wp, a new one is created
...
exports.Order = Backbone.Model.extend({
    initialize: function(attributes,options){
        ...
        this.wp_session_id = this.wp.wp_session.id;
        this.finalized      = false; // if true, cannot be modified.
        this.set_pricelist(this.wp.default_pricelist);
        ...
        if (options.json) {
            ...
            this.sequence_number = this.wp.wp_session.sequence_number++;
            this.uid  = this.generate_unique_id();
            this.name = _t("Order ") + this.uid;
            this.validation_date = undefined;
            this.fiscal_position = _.find(this.wp.fiscal_positions, function(fp) {
                return fp.id === self.wp.config.default_fiscal_position_id[0];
            });
        }
    ...
    save_to_db: function(){
        if (!this.temporary && !this.init_locked) {
            this.wp.db.save_unpaid_order(this);
        }
    },
    init_from_JSON: function(json) {
        var client;
        this.sequence_number = json.sequence_number;
        this.wp.wp_session.sequence_number = Math.max(this.sequence_number+1,this.wp.wp_session.sequence_number);
        this.session_id = json.wp_session_id;
        this.uid = json.uid;
        this.name = _t("Order ") + this.uid;
        this.validation_date = json.creation_date;

        if (json.fiscal_position_id) {
            var fiscal_position = _.find(this.wp.fiscal_positions, function (fp) {
            ...
            if (fiscal_position) {
                ...
                console.error('ERROR: trying to load a fiscal position not available in the wp');
            }
        }

        if (json.pricelist_id) {
            this.pricelist = _.find(this.wp.pricelists, function (pricelist) {
                return pricelist.id === json.pricelist_id;
            });
        } else {
            this.pricelist = this.wp.default_pricelist;
        }

        if (json.partner_id) {
            client = this.wp.db.get_partner_by_id(json.partner_id);
            if (!client) {
                console.error('ERROR: trying to load a partner not available in the wp');
            }
        } else {
            client = null;
        }
        ...
        for (var i = 0; i < orderlines.length; i++) {
            ...
            this.add_orderline(new exports.Orderline({}, {wp: this.wp, order: this, json: orderline}));
        }

        var paymentlines = json.statement_ids;
        for (var i = 0; i < paymentlines.length; i++) {
            var paymentline = paymentlines[i][2];
            var newpaymentline = new exports.Paymentline({},{wp: this.wp, order: this, json: paymentline});
    ...
    export_as_JSON: function() {
        ...
        return {
            ...
            wp_session_id: this.wp_session_id,
            pricelist_id: this.pricelist ? this.pricelist.id : false,
            partner_id: this.get_client() ? this.get_client().id : false,
            user_id: this.wp.get_cashier().id,
            uid: this.uid,
    ...
    export_for_printing: function(){
        ...
        var cashier = this.wp.get_cashier();
        var company = this.wp.company;
        var shop    = this.wp.shop;
        var date    = new Date();
        ...
        function render_xml(subreceipt){
            if (!is_xml(subreceipt)) {
                ...
                return qweb.render('subreceipt',{'wp':self.wp,'widget':self.wp.chrome,'order':self, 'receipt': receipt}) ;
            }
        }
        ...
        var receipt = {
            ...
            company:{
                ...
                name: company.name,
                phone: company.phone,
                logo:  this.wp.company_logo_base64,
            },
            shop:{
                name: shop.name,
            },
            currency: this.wp.currency,
        };

        if (is_xml(this.wp.config.receipt_header)){
            receipt.header = '';
            receipt.header_xml = render_xml(this.wp.config.receipt_header);
        } else {
            receipt.header = this.wp.config.receipt_header || '';
        }

        if (is_xml(this.wp.config.receipt_footer)){
            receipt.footer = '';
            receipt.footer_xml = render_xml(this.wp.config.receipt_footer);
        } else {
            receipt.footer = this.wp.config.receipt_footer || '';
        }
    ...
    generate_unique_id: function() {
        ...
        return zero_pad(this.wp.wp_session.id,5) +'-'+
               zero_pad(this.wp.wp_session.login_number,3) +'-'+
    ...
    get_tip: function() {
        var tip_product = this.wp.db.get_product_by_id(this.wp.config.tip_product_id[0]);
    ...
    fix_tax_included_price: function(line){
        if(this.fiscal_position){
            ...
            if (mapped_included_taxes.length > 0) {
                unit_price = line.compute_all(mapped_included_taxes, unit_price, 1, this.wp.currency.rounding, true).total_excluded;
            }
    ...
    add_product: function(product, options){
        if(this._printed){
            this.destroy();
            return this.wp.get_order().add_product(product, options);
        }
        this.assert_editable();
        options = options || {};
        ...
        attr.wp = this.wp;
        attr.order = this;
        var line = new exports.Orderline({}, {wp: this.wp, order: this, product: product});
    ...
    display_lot_popup: function() {
        ...
        if (order_line){
            ...
            this.wp.gui.show_popup('packlotline', {
    ...
    add_paymentline: function(cashregister) {
        this.assert_editable();
        var newPaymentline = new exports.Paymentline({},{order: this, cashregister:cashregister, wp: this.wp});
        if(cashregister.journal.type !== 'cash' || this.wp.config.iface_precompute_cash){
    ...
    get_tax_details: function(){
        ...
        for(var id in details){
            if(details.hasOwnProperty(id)){
                fulldetails.push({amount: details[id], tax: this.wp.taxes_by_id[id], name: this.wp.taxes_by_id[id].name});
            }
        }
    ...
    get_total_for_category_with_tax: function(categ_id){
        ...
        this.orderlines.each(function(line){
            if ( self.wp.db.category_contains(categ_id,line.product.id) ) {
    ...
    get_change: function(paymentline) {
        ...
        return round_pr(Math.max(0,change), this.wp.currency.rounding);
    },
    get_due: function(paymentline) {
        ...
        return round_pr(due, this.wp.currency.rounding);
    },
    is_paid: function(){
    ...
    destroy: function(){
        ...
        this.wp.db.remove_unpaid_order(this);
    },
    /* ---- Invoice --- */
    ...
    /* ---- Screen Status --- */
    // the order also stores the screen status, as the wp supports

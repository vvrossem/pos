...
// content of the right pane of the wp, containing the main functionalities. 
//
...
// to be used for the duration of the screen only. 
    ...
    var OrderWidget = WpBaseWidget.extend({
        ...
        init: function (parent, options) {
            ...
            this.wp.bind('change:selectedOrder', this.change_selected_order, this);
    ...
    var ProductCategoriesWidget = WpBaseWidget.extend({
        ...
        init: function (parent, options) {
            console.log('[ProductCategoriesWidget] init');
            var self = this;
            this._super(parent, options);
        ...
        set_category: function (category) {
            console.log('[ProductCategoriesWidget] set_category');
            var db = this.wp.db;
            if (!category) {
        ...
        render_category: function (category, with_image) {
            console.log('[ProductCategoriesWidget] render_category');
        ...
        replace: function ($target) {
            console.log('[ProductCategoriesWidget] replace');
            this.renderElement();
            var target = $target[0];
            target.parentNode.replaceChild(this.el, target);
        },
        ...
        renderElement: function () {
            console.log('[ProductCategoriesWidget] renderElement');
        ...
        reset_category: function () {
            console.log('[ProductCategoriesWidget] reset_category');
            this.set_category(this.wp.db.get_category_by_id(this.start_categ_id));
            this.renderElement();
        },
        ...
        clear_search: function () {
            console.log('[ProductCategoriesWidget] clear_search');
            var products = this.wp.db.get_product_by_category(this.category.id);
        ...
        perform_search: function (category, query, buy_result) {
            console.log('[ProductCategoriesWidget] perform_search');
            var products;
            if (query) {
    ...
    var ProductListWidget = WpBaseWidget.extend({
        ...
        renderElement: function () {
            console.log('[ProductListWidget] renderElement');
            var el_str = QWeb.render(this.template, {widget: this});
            var el_node = document.createElement('div');
            el_node.innerHTML = el_str;
            el_node = el_node.childNodes[1];
    ...
    var ClientListScreenWidget = ScreenWidget.extend({
        ...
        show: function () {
            ...
            this.old_client = this.wp.get_order().get_client();
            ...
            this.$('.new-customer').click(function () {
                self.display_client_details('edit', {
                    'country_id': self.wp.company.country_id,
                });
            });

            var partners = this.wp.db.get_partners_sorted(1000);
        ...
        barcode_client_action: function (code) {
            ...
            } else if (this.wp.db.get_partner_by_barcode(code.code)) {
                var partner = this.wp.db.get_partner_by_barcode(code.code);
        ...
        perform_search: function (query, associate_result) {
            var customers;
            if (query) {
                customers = this.wp.db.search_partner(query);
                ...
                customers = this.wp.db.get_partners_sorted();
                this.render_list(customers);
            }
        },
        clear_search: function () {
            var customers = this.wp.db.get_partners_sorted(1000);
            this.render_list(customers);
        ...
        save_changes: function () {
            ...
            if (this.has_client_changed()) {
                var default_fiscal_position_id = _.findWhere(this.wp.fiscal_positions, {'id': this.wp.config.default_fiscal_position_id[0]});
                if (this.new_client) {
                    if (this.new_client.property_account_position_id) {
                        var client_fiscal_position_id = _.findWhere(this.wp.fiscal_positions, {'id': this.new_client.property_account_position_id[0]});
                        order.fiscal_position = client_fiscal_position_id || default_fiscal_position_id;
                    }
                    order.set_pricelist(_.findWhere(this.wp.pricelists, {'id': this.new_client.property_product_pricelist[0]}) || this.wp.default_pricelist);
                } else {
                    order.fiscal_position = default_fiscal_position_id;
                    order.set_pricelist(this.wp.default_pricelist);
                }
        ...
        line_select: function (event, $line, id) {
            var partner = this.wp.db.get_partner_by_id(id);
        ...
        saved_client_details: function (partner_id) {
            ...
            this.reload_partners().then(function () {
                var partner = self.wp.db.get_partner_by_id(partner_id);
                if (partner) {
        ...
        reload_partners: function () {
            var self = this;
            return this.wp.load_new_partners().then(function () {
                // partners may have changed in the backend
                self.partner_cache = new DomCache();

                self.render_list(self.wp.db.get_partners_sorted(1000));

                // update the currently assigned client if it has been changed in db.
                var curr_client = self.wp.get_order().get_client();
                if (curr_client) {
                    self.wp.get_order().set_client(self.wp.db.get_partner_by_id(curr_client.id));
                }
            });
        },
        ...
        display_client_details: function (visibility, partner, clickwp) {
            var self = this;
            ...
            if (visibility === 'show') {
                ...
                if (!this.details_visible) {
                    ...
                    if (clickwp < scroll + new_height + 20) {
                        parent.scrollTop(clickwp - 20);
                    } else {
    ...
    // The receipt screen is not shown if the weighing point
    ...
    var ReceiptScreenWidget = ScreenWidget.extend({
        ...
        should_auto_print: function () {
            return this.wp.config.iface_print_auto && !this.wp.get_order()._printed;
        },
        should_close_immediately: function () {
            return this.wp.config.iface_print_via_proxy && this.wp.config.iface_print_skip_screen;
        },
        ...
        print_xml: function () {
            ...
            this.wp.proxy.print_receipt(receipt);
            this.wp.get_order()._printed = true;
        },
        print: function () {
            var self = this;

            if (!this.wp.config.iface_print_via_proxy) { // browser (html) printing
        ...
        render_change: function () {
            this.$('.change-value').html(this.format_currency(this.wp.get_order().get_change()));
        },
        render_receipt: function () {
            this.$('.wp-receipt-container').html(QWeb.render('WpTicket', this.get_receipt_render_env()));
        },
    });
    ...
    var PaymentScreenWidget = ScreenWidget.extend({
        ...
        reset_input: function () {
            var line = this.wp.get_order().selected_paymentline;
            this.firstinput = true;
            if (line) {
        ...
        click_numpad: function (button) {
            var paymentlines = this.wp.get_order().get_paymentlines();
            ...
            if (!open_paymentline) {
                this.wp.get_order().add_paymentline(this.wp.cashregisters[0]);
                this.render_paymentlines();
            }
        ...
        click_delete_paymentline: function (cid) {
            var lines = this.wp.get_order().get_paymentlines();
            for (var i = 0; i < lines.length; i++) {
                if (lines[i].cid === cid) {
                    this.wp.get_order().remove_paymentline(lines[i]);
                    this.reset_input();
                    this.render_paymentlines();
                    return;
                }
        ...
        click_paymentline: function (cid) {
            ...
            for (var i = 0; i < lines.length; i++) {
                if (lines[i].cid === cid) {
                    this.wp.get_order().select_paymentline(lines[i]);
                    this.reset_input();
                    this.render_paymentlines();
                    return;
                }
        ...
        click_paymentmethods: function (id) {
            ...
            for (var i = 0; i < this.wp.cashregisters.length; i++) {
                if (this.wp.cashregisters[i].journal_id[0] === id) {
                    cashregister = this.wp.cashregisters[i];
                    break;
                }
            }
            this.wp.get_order().add_paymentline(cashregister);
            this.reset_input();
            this.render_paymentlines();
        },
        ...
        renderElement: function () {
            ...
            this.$('.js_cashdrawer').click(function () {
                self.wp.proxy.open_cashbox();
            });

        },
        show: function () {
            this.wp.get_order().clean_empty_paymentlines();
            this.reset_input();
            this.render_paymentlines();
            this.order_changes();
            // that one comes from BarcodeEvents
            $('body').keypress(this.keyboard_handler);
            // that one comes from the wp, but we prefer to cover all the basis
        ...
        order_is_valid: function (force_validation) {
            ...
            if (Math.abs(order.get_total_with_tax() - order.get_total_paid()) > 0.00001) {
                var cash = false;
                for (var i = 0; i < this.wp.cashregisters.length; i++) {
                    cash = cash || (this.wp.cashregisters[i].journal.type === 'cash');
                }
                if (!cash) {
                    this.gui.show_popup('error', {
                        title: _t('Cannot return change without a cash payment method'),
                        body: _t('There is no cash payment method available in this weighing point to handle the change.\n\n Please pay the exact amount or add a cash payment method in the weighing point configuration'),
                    });
                    return false;
                }
            }
        ...
        finalize_validation: function () {
            ...
            if (order.is_paid_with_cash() && this.wp.config.iface_cashdrawer) {

                this.wp.proxy.open_cashbox();
            }
            ...
            if (order.is_to_invoice()) {
                var invoiced = this.wp.push_and_invoice_order(order);
                this.invoicing = true;
    ...
    var WeighingScreenWidget = ScreenWidget.extend({
        ...
        print: function () {
            ...
            /*if (!this.wp.config.iface_print_via_proxy) { //TODO(Vincent) create iface_label_printer instead?
                this.gui.show_popup('error', {
                    title: _t('No Label Printer'),
                    body: _t('Tick the box "Label Printer" in your IotBox / Hardware Proxy Configuration'),
                });
            } else {
                //TODO(Vincent) rpc
                //this.print_xml();
        ...
        get_container_weight:function(){
            ...
            return this.chrome.widget.scale_widget.get_weight();
        },

    });
    gui.define_screen({name: 'weighing', widget: WeighingScreenWidget});

    /* --------- The Selected Product Widget --------- */
    ...
    var SelectedProductWidget = WpBaseWidget.extend({
        ...
        renderElement: function () {
            console.log('[SelectedProductWidget] renderElement');
            var el_str = QWeb.render(this.template, {widget: this});
            var el_node = document.createElement('div');
            el_node.innerHTML = el_str;
            el_node = el_node.childNodes[1];
            ...
            this.el = el_node;
            if (this.product){
                var product_container = el_node.querySelector('.selected-product');
                var product_node = this.render_product(this.product);

odoo.define('weighing_point.devices', function (require) {
    "use strict";
    ...
    /**
     * The JobQueue schedules a sequence of 'jobs'.
     * Each job is a function returning a deferred.
     * The queue waits for each job to finish before launching the next.
     * Each job can also be scheduled with a delay.
    ...
    var JobQueue = function () {
        ...
        this.clear = function () {
            queue = _.filter(queue, function (job) {
                return job.opts.important === true;
            });
        };
        ...
        /**
         * returns a deferred that resolves when all scheduled jobs have been run.
     ...
     * connected to the Weighing Point. As the communication only goes from the WP to the proxy,
        ...
        print_sale_details: function () {
            ...
            rpc.query({
                ...
                .then(function (result) {
                    var env = {
                        widget: new WpBaseWidget(self),
                        company: self.wp.company,
                        wp: self.wp,
    ...
    /**
     * This module interfaces with the barcode reader.
     * It assumes the barcode reader is set-up to act like a keyboard.
     * Use connect() and disconnect() to activate and deactivate the barcode reader.
     * Use set_action_callbacks to tell it what to do when it reads a barcode.
     */

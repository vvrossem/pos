...
class WpSession(models.Model):
    _name = 'wp.session'
    _order = 'id desc'
    _description = 'weighing point Session'

    WP_SESSION_STATE = [
        ('opening_control', 'Opening Control'),  # method action_wp_session_open
        ('opened', 'In Progress'),               # method action_wp_session_closing_control
        ('closing_control', 'Closing Control'),  # method action_wp_session_close
    ...
    def _confirm_orders(self):
        for session in self:
            ...
            journal_id = self.env['ir.config_parameter'].sudo().get_param(
                'wp.closing.journal_id_%s' % company_id, default=session.config_id.journal_id.id)
            if not journal_id:
                raise UserError(_("You have to set a Sale Journal for the WP:%s") % (session.config_id.name,))

            move = self.env['wp.order'].with_context(force_company=company_id)._create_account_move(session.start_at, session.name, int(journal_id), company_id)
                    ...
                    raise UserError(
                        _("You cannot confirm all orders of this session, because they have not the 'paid' status.\n"
                          "{reference} is in state {state}, total amount: {total}, paid: {paid}").format(
                            reference=order.wp_reference or order.name,
                            state=order.state,
                            total=order.amount_total,
                            paid=order.amount_paid,
                        ))
                order.action_wp_order_done()
            orders_to_reconcile = session.order_ids._filtered_for_reconciliation()
            orders_to_reconcile.sudo()._reconcile_payments()

    config_id = fields.Many2one(
        'wp.config', string='weighing point',
        help="The physical weighing point you will use.",
        required=True,
        index=True)
    ...
    state = fields.Selection(
        WP_SESSION_STATE, string='Status',
    ...
    login_number = fields.Integer(string='Login Sequence Number', help='A sequence number that is incremented each time a user resumes the wp session', default=0)
    ...
    order_ids = fields.One2many('wp.order', 'session_id',  string='Orders')
    statement_ids = fields.One2many('account.bank.statement', 'wp_session_id', string='Bank Statement', readonly=True)
    ...
    _sql_constraints = [('uniq_name', 'unique(name)', "The name of this WP Session must be unique !")]

    @api.multi
    def _compute_picking_count(self):
        for wp in self:
            pickings = wp.order_ids.mapped('picking_id').filtered(lambda x: x.state != 'done')
            wp.picking_count = len(pickings.ids)
    ...
    def _check_wp_config(self):
        if self.search_count([
            ...
            raise ValidationError(_("Another session is already opened for this weighing point."))

    @api.model
    def create(self, values):
        config_id = values.get('config_id') or self.env.context.get('default_config_id')
        if not config_id:
            raise UserError(_("You should assign a weighing point to your session."))

        # journal_id is not required on the wp_config because it does not
        # exists at the installation. If nothing is configured at the
        # installation we do the minimal configuration. Impossible to do in
        # the .xml files as the CoA is not yet installed.
        wp_config = self.env['wp.config'].browse(config_id)
        ctx = dict(self.env.context, company_id=wp_config.company_id.id)
        if not wp_config.journal_id:
            default_journals = wp_config.with_context(ctx).default_get(['journal_id', 'invoice_journal_id'])
            if (not default_journals.get('journal_id') or
                    not default_journals.get('invoice_journal_id')):
                raise UserError(_("Unable to open the session. You have to assign a sales journal to your weighing point."))
            wp_config.with_context(ctx).sudo().write({
                'journal_id': default_journals['journal_id'],
                'invoice_journal_id': default_journals['invoice_journal_id']})
        # define some cash journal if no payment method exists
        if not wp_config.journal_ids:
            ...
            wp_config.sudo().write({'journal_ids': [(6, 0, journals.ids)]})

        wp_name = self.env['ir.sequence'].with_context(ctx).next_by_code('wp.session')
        if values.get('name'):
            wp_name += ' ' + values['name']

        statements = []
        ABS = self.env['account.bank.statement']
        uid = SUPERUSER_ID if self.env.user.has_group('weighing_point.group_wp_user') else self.env.user.id
        for journal in wp_config.journal_ids:
            ...
            ctx['journal_id'] = journal.id if wp_config.cash_control and journal.type == 'cash' else False
            st_values = {
        ...
        res = super(WpSession, self.with_context(ctx).sudo(uid)).create(values)
        if not wp_config.cash_control:
            res.action_wp_session_open()
    ...
    def unlink(self):
        ...
        return super(WpSession, self).unlink()
    ...
    @api.multi
    def action_wp_session_open(self):
    ...
    @api.multi
    def action_wp_session_closing_control(self):
        self._check_wp_session_balance()
        for session in self:
            session.write({'state': 'closing_control', 'stop_at': fields.Datetime.now()})
            if not session.config_id.cash_control:
                session.action_wp_session_close()

    @api.multi
    def _check_wp_session_balance(self):
        for session in self:
            for statement in session.statement_ids:
    ...
    @api.multi
    def action_wp_session_validate(self):
        self._check_wp_session_balance()
        self.action_wp_session_close()

    @api.multi
    def action_wp_session_close(self):
        # Close CashBox
        for session in self:
            ...
            for st in session.statement_ids:
                if abs(st.difference) > st.journal_id.amount_authorized_diff:
                    # The wp manager can close statements with maximums.
                    if not self.user_has_groups("weighing_point.group_wp_manager"):
        ...
        return {
            ...
            'name': 'weighing point Menu',
            'tag': 'reload',
            'params': {'menu_id': self.env.ref('weighing_point.menu_point_root').id},
        }
    ...
    def open_frontend_cb(self):
        ...
        for session in self.filtered(lambda s: s.user_id.id != self.env.uid):
            raise UserError(_("You cannot use the session of another user. This session is owned by %s. "
                              "Please first close this one to use this weighing point.") % session.user_id.name)
        return {
    ...
    def open_cashbox(self):
        ...
        context['default_wp_id'] = self.config_id.id

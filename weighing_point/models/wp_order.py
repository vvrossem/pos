...
class WpOrder(models.Model):
    _name = "wp.order"
    _description = "weighing point Orders"
    _order = "id desc"
    ...
    def _order_fields(self, ui_order):
        process_line = partial(self.env['wp.order.line']._order_line_fields, session_id=ui_order['wp_session_id'])
        return {
            'name':         ui_order['name'],
            'user_id':      ui_order['user_id'] or False,
            'session_id':   ui_order['wp_session_id'],
            'lines':        [process_line(l) for l in ui_order['lines']] if ui_order['lines'] else False,
            'wp_reference': ui_order['name'],
    ...
    def _get_valid_session(self, order):
        ...
        closed_session = WpSession.browse(order['wp_session_id'])
        ...
        _logger.warning('session %s (ID: %s) was closed but received order %s (total: %s) belonging to it',
                        ...
                        order['name'],
                        order['amount_total'])
        rescue_session = WpSession.search([
    ...
    def _match_payment_to_invoice(self, order):
        ...
        # ignore orders with an amount_paid of 0 because those are returns through the wp
    ...
    @api.model
    def _process_order(self, wp_order):
        wp_session = self.env['wp.session'].browse(wp_order['wp_session_id'])
        if wp_session.state == 'closing_control' or wp_session.state == 'closed':
            wp_order['wp_session_id'] = self._get_valid_session(wp_order).id
        order = self.create(self._order_fields(wp_order))
        prec_acc = order.pricelist_id.currency_id.decimal_places
        journal_ids = set()
        for payments in wp_order['statement_ids']:
            if not float_is_zero(payments[2]['amount'], precision_digits=prec_acc):
                order.add_payment(self._payment_fields(payments[2]))
            journal_ids.add(payments[2]['journal_id'])

        if wp_session.sequence_number <= wp_order['sequence_number']:
            wp_session.write({'sequence_number': wp_order['sequence_number'] + 1})
            wp_session.refresh()

        if not float_is_zero(wp_order['amount_return'], prec_acc):
            cash_journal_id = wp_session.cash_journal_id.id
            if not cash_journal_id:
                ...
                if not cash_journal:
                    # If none, select for change one of the cash journals of the wp
                    # This is used for example when a customer pays by credit card
                    # an amount higher than total amount of the order and gets cash back
                    cash_journal = [statement.journal_id for statement in wp_session.statement_ids if statement.journal_id.type == 'cash']
                    if not cash_journal:
                        raise UserError(_("No cash statement found for this session. Unable to record returned cash."))
                cash_journal_id = cash_journal[0].id
            order.add_payment({
                'amount': -wp_order['amount_return'],
    ...
    def _prepare_invoice(self):
        """
        Prepare the dict of values to create the new invoice for a wp order.
        """
        ...
        def add_anglosaxon_lines(grouped_data):
            ...
            for product_key in list(grouped_data.keys()):
                if product_key[0] == "product":
                    ...
                    price_unit = self._get_wp_anglo_saxon_price_unit(product, line['partner_id'], line['quantity'])
        ...
        for order in self.filtered(lambda o: not o.account_move or o.state == 'paid'):
            ...
            if move is None:
                ...
                journal_id = self.env['ir.config_parameter'].sudo().get_param(
                    'wp.closing.journal_id_%s' % current_company.id, default=order.sale_journal.id)
            ...
            # because of the weird way the wp order is written, we need to make sure there is at least one line,
            # because just after the 'for' loop there are references to 'line' and 'income_account' variables (that
            # are set inside the for loop)
            # TOFIX: a deep refactoring of this method (and class!) is needed
            # in order to get rid of this stupid hack
            assert order.lines, _('The wp order must have lines when calling this method')
    ...
    def _get_wp_anglo_saxon_price_unit(self, product, partner_id, quantity):
    ...
    def _filtered_for_reconciliation(self):
        ...
        if self.env['ir.config_parameter'].sudo().get_param('weighing_point.order_reconcile_mode', 'all') == 'partner_only':
    ...
    def _default_session(self):
        return self.env['wp.session'].search([('state', '=', 'opened'), ('user_id', '=', self.env.uid)], limit=1)
    ...
    lines = fields.One2many('wp.order.line', 'order_id', string='Order Lines', states={'draft': [('readonly', False)]}, readonly=True, copy=True)
    statement_ids = fields.One2many('account.bank.statement.line', 'wp_statement_id', string='Payments', states={'draft': [('readonly', False)]}, readonly=True)
    ...
    session_id = fields.Many2one(
        'wp.session', string='Session', required=True, index=True,
        domain="[('state', '=', 'opened')]", states={'draft': [('readonly', False)]},
        readonly=True, default=_default_session)
    config_id = fields.Many2one('wp.config', related='session_id.config_id', string="weighing point", readonly=False)
    ...
    wp_reference = fields.Char(string='Receipt Ref', readonly=True, copy=False)
    ...
    def _onchange_amount_all(self):
        ...
        for order in self.env['account.bank.statement.line'].read_group([('wp_statement_id', 'in', self.ids)], ['wp_statement_id', 'amount'], ['wp_statement_id']):
            amounts[order['wp_statement_id'][0]]['paid'] = order['amount']
        for order in self.env['account.bank.statement.line'].read_group(['&', ('wp_statement_id', 'in', self.ids), ('amount', '<', 0)], ['wp_statement_id', 'amount'], ['wp_statement_id']):
            amounts[order['wp_statement_id'][0]]['return'] = order['amount']
        for order in self.env['wp.order.line'].read_group([('order_id', 'in', self.ids)], ['order_id', 'price_subtotal', 'price_subtotal_incl'], ['order_id']):
    ...
    @api.multi
    def write(self, vals):
        res = super(WpOrder, self).write(vals)
        Partner = self.env['res.partner']
        # If you change the partner of the wp order, change also the partner of the associated bank statement lines
        if 'partner_id' in vals:
            for order in self:
                partner_id = False
                if order.invoice_id:
                    raise UserError(_("You cannot change the partner of a wp order for which an invoice has already been issued."))
                if vals['partner_id']:
    ...
    @api.multi
    def unlink(self):
        for wp_order in self.filtered(lambda wp_order: wp_order.state not in ['draft', 'cancel']):
            raise UserError(_('In order to delete a sale, it must be new or cancelled.'))
        return super(WpOrder, self).unlink()

    @api.model
    def create(self, values):
        if values.get('session_id'):
            # set name based on the sequence specified on the config
            session = self.env['wp.session'].browse(values['session_id'])
            values['name'] = session.config_id.sequence_id._next()
            values.setdefault('pricelist_id', session.config_id.pricelist_id.id)
        else:
            # fallback on any wp.order sequence
            values['name'] = self.env['ir.sequence'].next_by_code('wp.order')
        return super(WpOrder, self).create(values)
    ...
    @api.multi
    def action_wp_order_invoice(self):
        ...
        for order in self:
            ...
            message = _("This invoice has been created from the weighing point session: <a href=# data-oe-model=wp.order data-oe-id=%d>%s</a>") % (order.id, order.name)
    ...
    @api.multi
    def action_wp_order_cancel(self):
        return self.write({'state': 'cancel'})

    @api.multi
    def action_wp_order_done(self):
    ...
    def create_from_ui(self, orders):
        ...
        wp_order = self.search([('wp_reference', 'in', submitted_references)])
        existing_orders = wp_order.read(['wp_reference'])
        existing_references = set([o['wp_reference'] for o in existing_orders])
        ...
        for tmp_order in orders_to_save:
            ...
            wp_order = self._process_order(order)
            order_ids.append(wp_order.id)

            try:
                wp_order.action_wp_order_paid()
            except psycopg2.OperationalError:
                # do not hide transactional errors, the order(s) won't be saved!
                raise
            except Exception as e:
                _logger.error('Could not fully process the wp Order: %s', tools.ustr(e))

            if to_invoice:
                wp_order.action_wp_order_invoice()
                wp_order.invoice_id.sudo().action_invoice_open()
                wp_order.account_move = wp_order.invoice_id.move_id
        return order_ids

    def test_paid(self):
        """A weighing point is paid when the sum
        @return: True
        """
        for order in self:
    ...
    def create_picking(self):
        ...
        for order in self:
            ...
            if picking_type:
                message = _("This transfer has been created from the weighing point session: <a href=# data-oe-model=wp.order data-oe-id=%d>%s</a>") % (order.id, order.name)
                picking_vals = {
                    'origin': order.name,
                ...
                wp_qty = any([x.qty > 0 for x in order.lines if x.product_id.type in ['product', 'consu']])
                if wp_qty:
            ...
            # when the wp.config has no picking_type_id set only the moves will be created
    ...
    def set_pack_operation_lot(self, picking=None):
        ...
        WpPackOperationLot = self.env['wp.pack.operation.lot']
        ...
        for order in self:
            for move in (picking or self.picking_id).move_lines:
                ...
                qty_done = 0
                pack_lots = []
                wp_pack_lots = WpPackOperationLot.search([('order_id', '=', order.id), ('product_id', '=', move.product_id.id)])

                if wp_pack_lots and lots_necessary:
                    for wp_pack_lot in wp_pack_lots:
                        stock_production_lot = StockProductionLot.search([('name', '=', wp_pack_lot.lot_name), ('product_id', '=', move.product_id.id)])
                        if stock_production_lot:
                            # a serialnumber always has a quantity of 1 product, a lot number takes the full quantity of the order line
                            qty = 1.0
                            if stock_production_lot.product_id.tracking == 'lot':
                                qty = wp_pack_lot.wp_order_line_id.qty
                            qty_done += qty
    ...
    def _prepare_bank_statement_line_payment_values(self, data):
        ...
        context.pop('wp_session_id', False)
    ...
    def refund(self):
        ...
        WpOrder = self.env['wp.order']
        current_session = self.env['wp.session'].search([('state', '!=', 'closed'), ('user_id', '=', self.env.uid)], limit=1)
        ...
        for order in self:
            clone = order.copy({
                ...
                'wp_reference': order.wp_reference,
                'lines': False,
...
class WpOrderLine(models.Model):
    _name = "wp.order.line"
    _description = "weighing point Order Lines"
    _rec_name = "product_id"

    def _order_line_fields(self, line, session_id=None):
        if line and 'name' not in line[2]:
            session = self.env['wp.session'].browse(session_id).exists() if session_id else None
            if session and session.config_id.sequence_line_id:
                # set name based on the sequence specified on the config
                line[2]['name'] = session.config_id.sequence_line_id._next()
            else:
                # fallback on any wp.order.line sequence
                line[2]['name'] = self.env['ir.sequence'].next_by_code('wp.order.line')
    ...
    order_id = fields.Many2one('wp.order', string='Order Ref', ondelete='cascade')
    tax_ids = fields.Many2many('account.tax', string='Taxes', readonly=True)
    tax_ids_after_fiscal_position = fields.Many2many('account.tax', compute='_get_tax_ids_after_fiscal_position', string='Taxes to Apply')
    pack_lot_ids = fields.One2many('wp.pack.operation.lot', 'wp_order_line_id', string='Lot/serial Number')
    ...
    def create(self, values):
        if values.get('order_id') and not values.get('name'):
            ...
            # cf TODO master is wp.config create
            # remove me saas-15
            self.env.cr.execute("""
                SELECT s.id
                FROM ir_sequence s
        ...
        if not values.get('name'):
            # fallback on any wp.order sequence
            values['name'] = self.env['ir.sequence'].next_by_code('wp.order.line')
        return super(WpOrderLine, self).create(values)
...
class WpOrderLineLot(models.Model):
    _name = "wp.pack.operation.lot"
    _description = "Specify product lot/serial number in wp order line"

    wp_order_line_id = fields.Many2one('wp.order.line')
    order_id = fields.Many2one('wp.order', related="wp_order_line_id.order_id", readonly=False)
    lot_name = fields.Char('Lot Name')
    product_id = fields.Many2one('product.product', related='wp_order_line_id.product_id', readonly=False)


class ReportSaleDetails(models.AbstractModel):

    _name = 'report.weighing_point.report_saledetails'
    _description = 'weighing point Details'
    ...
    def get_sale_details(self, date_start=False, date_stop=False, configs=False):
        ...
        """
        if not configs:
            configs = self.env['wp.config'].search([])
        ...
        orders = self.env['wp.order'].search([
        ...
        st_line_ids = self.env["account.bank.statement.line"].search([('wp_statement_id', 'in', orders.ids)]).ids
        if st_line_ids:
            self.env.cr.execute("""
    ...
    def _get_report_values(self, docids, data=None):
        data = dict(data or {})
        configs = self.env['wp.config'].browse(data['config_ids'])

...
class AccountTax(models.Model):
    ...
    def write(self, vals):
        ...
        if forbidden_fields & set(vals.keys()):
            tax_ids = self.env['wp.order.line'].sudo().search([
            ...
            if tax_ids & set(self.ids):
                raise UserError(_(
                    'It is forbidden to modify a tax used in a wp order not posted. ' +
                    'You must close the wp sessions before modifying the tax.'
                ))
